 ...
 */
class Encoder(override var name: String): DeviceAccessor("encoder$name") {
    ...
    val encoder: DcMotor
        get() {
            return HardwareGetter.getEncoderFromRequest("encoder$name")
        }
    ...
    val position: Int
        get() {
            return HardwareGetter.issueEncoderRequest("encoder$name", EncoderInput.GET)
        }
     ...
     */
    fun reset() {
        HardwareGetter.issueEncoderRequest("encoder$name", EncoderInput.SET)
    }
